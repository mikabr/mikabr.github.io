...
I'm a research software developer at the [Language and Cognition Lab](https://langcog.stanford.edu/) at Stanford University. As part of my role, I do data engineering and modeling for [LEVANTE](https://levante-network.org/), a global research network for coordinated data collection on children's learning and development. I also work on tools and resources for **data sharing** and other **open science practices**.
...
section_tags <- function(sec) {
  ...
  per_col <- ceiling(n_items / 3)
  tagList(
    HTML(glue(
      '<h3><i class="fa fa-{sec$icon} fa-color-{sec$color}"></i>{sec$title}</h3>'
    )),
    fluidRow(column(width = 4, sec_items[1:per_col]),
             column(width = 4, sec_items[(per_col + 1):(per_col + per_col)]),
             column(width = 4, sec_items[(per_col + per_col + 1):n_items]))
