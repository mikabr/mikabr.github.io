...
\moveleft.5\hoffset\centerline{ $if(email)$$if(fontawesome)$\faEnvelope \hspace{1 mm}$else$\emph{E-mail:}$endif$ \href{mailto:}{\tt $email$} \hspace{2 mm}$endif$ $if(github)$$if(fontawesome)$\faGithub \hspace{1 mm}$else$\emph{Github:}$endif$ \href{http://github.com/$github$}{\tt $github$} \hspace{2 mm} $endif$   $if(web)$$if(fontawesome)$\faGlobe \hspace{1 mm}$else$\emph{Web:}$endif$ \href{http://$web$}{\tt $web$}  \hspace{2 mm} $endif$   $if(phone)$$if(fontawesome)$ \faMobilePhone \hspace{1 mm}$else$\emph{Phone:}$endif$  {\tt $phone$}  \hspace{2 mm} $endif$   $if(updated)$ | \emph{Updated:} \today$endif$}
