...
\moveleft.5\hoffset\centerline{ $if(email)$$if(fontawesome)$\scalebox{1.3}{\faEnvelope}$else$\emph{E-mail:}$endif$ \href{mailto:}{\tt $email$} \hspace{1 mm}$endif$ $if(github)$$if(fontawesome)$\scalebox{1.4}{\faGithub}$else$\emph{Github:}$endif$ \href{http://github.com/$github$}{\tt $github$} \hspace{1 mm} $endif$   $if(web)$$if(fontawesome)$\scalebox{1.4}{\faGlobe}$else$\emph{Web:}$endif$ \href{http://$web$}{\tt $web$}  \hspace{1 mm} $endif$   $if(phone)$$if(fontawesome)$ \scalebox{1.4}{\faMobilePhone}$else$\emph{Phone:}$endif$  {\tt $phone$}  \hspace{1 mm} $endif$} %   $if(updated)$ | \emph{Updated:} \today$endif$}
